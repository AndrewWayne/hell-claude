...
`hooks/phrases.json` 保存共享词表。macOS 和 Linux 使用 `detect-complaint.sh`，Windows 使用 `detect-complaint.ps1`。两个脚本遵守相同的 stdin、stdout 和退出码协议，并读取同一份词表。`hooks.json` 为 Codex 配置 Windows 命令覆盖；Claude Code 的安装配置选择当前系统对应的命令。规格中的“一个 Hook”指一套行为与协议，不要求所有操作系统执行同一个脚本。
