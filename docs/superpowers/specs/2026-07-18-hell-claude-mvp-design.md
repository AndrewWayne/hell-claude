...
状态：MVP 已实施；以仓库测试和 `docs/evals/mvp-acceptance.md` 为验收依据
...
MVP 提供 Codex 和 Claude Code 的直接安装方式。两者共用 Hook 行为、`SKILL.md`、报告 schema、词表和提交规则。安装清单与 Hook 命令保留平台适配。
...
1. 用户本机：Hook 向 Agent 注入调用 Skill 的指令；Skill 选择上下文、脱敏并生成预览。用户确认前，报告内容不离开本机。
...
Hook 只判断是否需要调用 Skill。它不读取工作区、不生成报告，也不提交内容。`UserPromptSubmit` 事件在 Codex 和 Claude Code 中都不支持 Prompt matcher，因此 Hook 在每次用户提交 Prompt 时运行，并在自己的进程内完成文本匹配。

Hook 接收平台通过 stdin 传入的 JSON，从 `prompt` 字段读取用户输入。命中规则时，Hook 输出一条简短的附加上下文，要求 Agent 调用 `hell-report` Skill；未命中时不输出内容并返回成功。Hook 不阻断用户 Prompt。
...
`plugins/hell-claude/hooks/phrases.json` 保存共享词表。macOS 和 Linux 使用 `detect-complaint.sh`，Windows 使用 `detect-complaint.ps1`。两个脚本遵守相同的 stdin、stdout 和退出码协议，并读取同一份词表。`hooks.json` 提供 POSIX 命令和 Windows 命令覆盖。规格中的“一个 Hook”指一套行为与协议，不要求所有操作系统执行同一个脚本。

MVP 不使用 prompt-based Hook。Claude Code 支持这种 Hook，Codex 当前只执行 command Hook；使用 command Hook 可以让两个客户端共享行为。
...
- Hook 契约测试用相同 fixture 验证 POSIX 和 PowerShell 实现对 `/hell`、命中词、普通 Prompt、无效 JSON 和冷却状态产生相同结果。
- 插件端到端测试分别覆盖 Codex 和 Claude Code 的自动触发、`/hell`、取消、编辑、确认、`gh` 提交和浏览器回退，并在 Windows 环境验证 PowerShell Hook。
...
- 两种客户端在 macOS、Linux 和 Windows 上的安装、升级和卸载验证；
- 本地不确认不上传的端到端验证；
