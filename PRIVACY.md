...
The Hook reads the current submitted prompt, its packaged phrase list, and local cooldown/configuration state. It does not read repository files or contact a network service. A match asks the Agent to run the Skill; it is not permission to submit.
